/**
 * Standalone High-Density QR Code Generator (TypeNumber 1 to 40)
 * Suporta textos e URLs longas (até 3.000 caracteres) em modo Byte (8-bit)
 * Funciona 100% offline via Canvas / SVG nativo.
 */
...
// Polynomial and Math for Reed-Solomon Error Correction
const QRMath = {
  glog(n) {
...
// RS Block and QR Capacity specifications (Type 1 to 40)
const QRRSBlock = {
  RS_BLOCK_TABLE: [
...
/**
 * Função utilitária para renderizar o QR Code diretamente em um elemento HTML (Canvas)
 */
export function generateQRCodeCanvas(text, size = 180) {
  ...
  canvas.width = size;
  canvas.height = size;
  canvas.className = 'w-full h-full rounded-lg shadow-md';

  const ctx = canvas.getContext('2d');
  const cellSize = size / count;

  // Fundo branco com margem
  ctx.fillStyle = '#ffffff';
  ctx.fillRect(0, 0, size, size);

  // Desenha os módulos pretos
